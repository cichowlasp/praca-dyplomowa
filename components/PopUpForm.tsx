...
const PopUpForm = ({
	...
	>([]);
	const [initIndex, setInitalIndex] = useState(startIndex);
	...
	const handleClick = async () => {
		...
		setError(validForm([...mainData, ...data]));
		if (validForm([...mainData, ...data]).length !== 0) {
			return;
		}
		setFormDataMain((pre) => {
			pre[ind].popup = [...mainData, ...data];
			return pre;
		});
		closeForm();
	};
